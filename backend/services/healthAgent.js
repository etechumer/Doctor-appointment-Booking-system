...
Never explain. Never write extra text.
  ...
  try {
    ...
    const res = (
      typeof raw === "string" ? raw : raw?.message || raw?.output || ""
    )
      .toLowerCase()
      .trim();
...
export async function handleHealthQuery(message) {
  ...
  if (greetings.some((g) => text.includes(g))) {
    return {
      answer:
        "👋 Hello! I’m HealthBot — your friendly assistant for general health and wellness information. How can I help you today?",
      source: "general",
    };
  }
  ...
  try {
    ...
    const reply =
      (typeof rawReply === "string"
        ? rawReply
        : rawReply?.message || rawReply?.output || ""
