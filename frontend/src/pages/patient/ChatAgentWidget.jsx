  ...
  const send = async () => {
    ...
    try {
      const { data } = await axios.post(
        "http://localhost:4000/api/patient/agent/chat",
        {
          message: user.text,
        },
        {
      ...
      setMsgs((m) => [
        ...m,
        { sender: "bot", text: data.reply || "Sorry, I didn't understant" },
      ]);
    } catch (error) {
      setMsgs((m) => [
        ...m,
  ...
  return (
    <>
      <button
        ...
        className="fixed bottom-5 right-5 bg-blue-600 hover:bg-blue-700 text-white p-4 rounded shadow-lg cursor-pointer"
      >
      ...
      {open && (
        <div className="fixed bottom-20 right-5 w-80 sm:w-96 bg-white rounded-2xl shadow-xl border flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto bg-gray-50 p-3 text-sm max-h-80">
            ...
            {loading && (
              <p className="text-gray-500 italic text-center mt-2 text-xs">
                Thinking...
              </p>
            )}
          </div>
