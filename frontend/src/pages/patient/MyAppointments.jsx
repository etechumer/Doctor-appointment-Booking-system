...
const MyAppointments = () => {
    ...
    const fetchMyAppointments = async () => {
      try {
        const res = await axios.get(
          "http://localhost:4000/api/patient/myAppointments",
  ...
  if (!appointments.length)
    return (
      <div className="m-4 rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-10 text-center text-slate-500">
        ...
        {appointments.map((appt) => {
          ...
          return (
            <button
              ...
              onClick={() =>
                navigate(
                  `/patient-page/my-appointments/${appt.appointmentId}`,
                  { state: { appointment: appt } },
                )
              }
              className="group relative flex min-h-68 flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white p-5 text-left shadow-[0_8px_24px_-18px_rgba(15,23,42,0.4)] transition-all duration-300 ease-out hover:-translate-y-1 hover:border-blue-200 hover:shadow-[0_18px_35px_-20px_rgba(37,99,235,0.45)] focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <span
                  className={`rounded-full px-2.5 py-1 text-[11px] font-bold tracking-wide ring-1 ring-inset ${statusStyles[appt.status] || "bg-slate-100 text-slate-600 ring-slate-200"}`}
                >
                  {appt.status}
                </span>
              </div>
              <div className="mt-5">
                <p className="text-xs font-medium text-slate-400">
                  DR.{" "}
                  {appt.doctor?.specialization?.toUpperCase() || "CONSULTATION"}
                </p>
                <h3 className="mt-1 text-lg font-bold text-slate-900">
                  {appt.doctor?.name || "Assigned doctor"}
                </h3>
                <p className="mt-1 text-sm text-slate-500">
                  {appt.doctor?.specialization || "Medical specialist"}
                </p>
              </div>
