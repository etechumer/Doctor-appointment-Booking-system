...
const ChatWindow = ({ appointmentId, receiverId }) => {
  ...
  const fetchMessages = async () => {
    try {
      const res = await axios.get(
        `http://localhost:4000/api/auth/getMessages/${appointmentId}`,
        {
          headers: {
  ...
  const handleSend = async () => {
    ...
    try {
      await axios.post(
        "http://localhost:4000/api/auth/sendMessage",
        {
          appointmentId,
          receiverId,
          message: newMsg.trim(),
        ...
        {!isOpen ? (
          <div className="text-center">
            ...
            <button
              ...
              className="bg-blue-600 text-white px-4 py-1.5 rounded-md text-sm font-medium hover:bg-blue-700 cursor-pointer"
            >
              Open Chat
            </button>
          </div>
          ...
          <>
                ...
                {messages.length ? (
                  messages.map((msg) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`max-w-[70%] p-2 rounded-lg text-sm shadow-sm ${
                            isSender
            ...
            <div className="flex gap-2 mt-3">
              ...
              <button
                onClick={handleSend}
                className="bg-blue-600 text-white px-4 rounded-md text-sm cursor-pointer hover:bg-blue-700"
              >
                Send
              </button>
            </div>
